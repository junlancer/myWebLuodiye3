...
class ProductController extends CommonContentController {
	...
	public function addPost() {
		...
		if ($id = M('product')->add($data)) {
			...
			//SearchAll
			$data_search             = array('arc_id' => $id, 'title' => $data['title'], 'slt' => $data['slt'],'litpic' => $data['litpic'], 'description' => $data['description'], 'jump_url' => $data['jump_url'], 'publish_time' => $data['publish_time'], 'cid' => $data['cid'], 'model_id' => $model_id, 'point' => $data['point'], 'delete_status' => $data['delete_status'], 'audit_status' => 1);
